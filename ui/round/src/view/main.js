...
module.exports = function(ctrl) {
  ...
  if (ctrl.bugController){
      var bd = ctrl.bugController.data,
        bugTopColor = bd[ctrl.bugController.vm.flip ? 'player' : 'opponent'].color,
        bugBottomColor = bd[ctrl.bugController.vm.flip ? 'opponent' : 'player'].color,
        bugCgState = ctrl.bugController.chessground && ctrl.bugController.chessground.state,
        bugMaterial, bugScore;
      
      if (bd.pref.showCaptured) {
        var pieces = bugCgState ? bugCgState.pieces : fenRead(plyStep(bd, ctrl.bugController.vm.ply).fen);
        bugMaterial = util.getMaterialDiff(pieces);
        bugScore = util.getScore(pieces) * (bottomColor === 'white' ? 1 : -1);
      } else bugMaterial = emptyMaterialDiff;
      
      boards.push(d.blind ? blindBoard(ctrl.bugController) : visualBoard(ctrl.bugController),
        m('div.lichess_ground', [
          crazyView.pocket(ctrl.bugController, bugTopColor, 'top') || renderMaterial(ctrl.bugController, material[topColor], bd.player.checks),
          renderTable(ctrl.bugController),
          crazyView.pocket(ctrl.bugController, bugBottomColor, 'bottom') || renderMaterial(ctrl.bugController, material[bottomColor], bd.opponent.checks, bugScore)
        ])
      )
  }
